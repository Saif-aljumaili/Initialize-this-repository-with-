clear all
clc
header = blockEdfLoad('s14.edf');
[header, signalHeader, signalCell] = blockEdfLoad('s14.edf'); 
%%signal without decomposition 
% for i=1:19 
%     Saif(i,:)=signalCell{i};
% end
% p=plot(Saif(:,1:100)');
% title('EEG Signal')

%decomposition signal using bandpass filter
for i=1:19 
    Saif(i,:)=signalCell{i};
    Fs=250;
    dalta(i,:)=bandpass(Saif(i,:),[0.1,4],Fs);
    theta(i,:)=bandpass(Saif(i,:),[5,9],Fs);
    aplha(i,:)=bandpass(Saif(i,:),[10,14],Fs);
    beta(i,:)=bandpass(Saif(i,:),[15,31],Fs);
    gama(i,:)=bandpass(Saif(i,:),[32,100],Fs);
end

% subplot(5,1,1)
% p=plot(dalta(:,1:100)');
% title('Dalta Signal')
% subplot(5,1,2)
% p=plot(theta(:,1:100)');
% title('Theta Signal')
% subplot(5,1,3)
% p=plot(aplha(:,1:100)');
% title('Aplha Signal')
% subplot(5,1,4)
% p=plot(beta(:,1:100)');
% title('Beta Signal')
% subplot(5,1,5)
% p=plot(gama(:,1:100)');
% title('Gama Signal')


% start = fix(250*2.1324);
% stop = fix(250*50.5636);
data = dalta(:,:)';
data1 = theta(:,:)';
data2 = aplha(:,:)';
data3 = beta(:,:)';
data4 =gama(:,:)';
coun=0;
for k=1:500:size(data,1)-499
    k
    coun=coun+1;
     for  j=1:19
         x=data(k:k+249,j);
         x1=data1(k:k+249,j);
         x2=data2(k:k+249,j);
         x3=data3(k:k+249,j);
         x4=data4(k:k+249,j);
%           r=0.2*std(x);
%           r1=0.2*std(x1);
%           r2=0.2*std(x2);
%           r3=0.2*std(x3);
%           r4=0.2*std(x4);
         apen = kurtosis(x);
         apen1 = kurtosis(x1);
         apen2 = kurtosis(x2);
         apen3 = kurtosis(x3);
         apen4 = kurtosis(x4);
         DData(coun,5*(j-1)+1:5*j)=[apen apen1 apen2 apen3 apen4];
     end 
end 
xlswrite('s14_kurtosis.xlsx',[DData]);



















t.trackID=SchizophreniaBandpassFilter2Classes
rowsToPlot = (1) || ( 2) || (3)
x = t(rowsToPlot);
y = t(rowsToPlot);
z = t(rowsToPlot);
scatter(x, y, z);
grid on;
xlabel('x');
ylabel('y');
zlabel('time')



scatter3(SchizophreniaFFT2Classes,2,3,5,'o');

x=(1:1624,:)
y=1624:3256,:


x = (saiff(1:1624,:));
y = (saiff(1624:3256,:));
z = (saiff(3256:5243:));
q=(saiff(5246:7022,:)
s = x(:);
c = y(:);
g = z(:);

scatter3(x(:),y(:),z(:))

scatter3(shufflenet,1,16,32)

x= shufflenet(4,5);
y=shufflenet(4,6);
z=shufflenet(8,10)


gscatter(x,y,z)


s = scatter3(shufflenet,{2,5,7},'filled','ColorVariable','c');


scatter3(shufflenet,20,50,70,'MarkerEdgeColor','c', 'MarkerFaceColor',[0.2 .75 .10]);
view(-30,10)

X = [1:20];
Y = [20:40];
Z = [40:80];
scatter3(X,Y,Z)

C = [1 0 0; 0.6 0 1];
X = [1 2 3 4];
Y = [5 6 7 8];
Z = [2 5; 1 2; 8 4; 11 9];
S = 50;
scatter3(X,Y,Z,S,C)
